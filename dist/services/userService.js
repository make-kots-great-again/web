...
function userServiceFactory({
  ...
  async function listOneUser({
    ...
    if (!id.match(/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i)) return {
      message: `${id} is not a valid v4 UUID`
    };
  ...
  async function patchUserPwd({
    ...
    const newPassword = _security.passwordFactory.hashPassword(userInfo.newPassword);

    if (!(await _security.passwordFactory.verifyPassword(userInfo.password, user.password))) return {
      message: `wrong password`
    };
    return await userRepository.patchPwd({
      id
    }, {
      newPassword
    });
  ...
  async function searchUser({
    ...
    return await userRepository.searchUsername({
      username
    });
  }
}