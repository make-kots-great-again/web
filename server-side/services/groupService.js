...
export default function groupServiceFactory ({ groupRepository, userRepository }) {
  ...
  async function addMembersToGroup ({ groupId, username }) {
    ...
    if (typeof username !== 'string') { return { message: 'A username must be a string.' } }

    if (username.length < 4 || username.length >= 32) { return { message: 'A username length must be between 4 and 32.' } }

    if (!(groupId.match(/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i))) { return { message: `${groupId} is not a valid v4 UUID` } }
    ...
    if (existingMember) { return { message: `${username} is already in this group !` } }
  ...
  async function deleteUserFromGroup ({ groupId, userId }) {
    ...
    if (deleteUser === 0) { return { message: 'No user with such id was found in this group !' } }
