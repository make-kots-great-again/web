export default function buildMakeShoppingList(requiredParameter) {
    return ({
        ...
        if (typeof quantity !== 'number')
            throw new TypeError("A product's quantity must be a number.");

        if (quantity < 1 || quantity > 20)
            throw new RangeError("A product's quantity must be between 1 and 20.");

        if (typeof groupProduct !== 'boolean')
            throw new TypeError("A groupProduct must be a boolean.");
